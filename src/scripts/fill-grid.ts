...
export const fillGrid = (options: Partial<IFillGridOptions> = {}): void => {
  ...
  document.querySelectorAll('.bfg__box, [bfg__box]').forEach((element) => fillElement(element, opts));
  document.querySelectorAll('.bfg-card__content, [bfg-card__content]').forEach((element) => fillElement(element, opts));
};
